...
var batterydao={
    sqlGetAll: "select * from battery_tbl WHERE datetime > DATE_SUB(NOW(), INTERVAL 1 DAY);",
    ...
    getAll:function(callback) {
        console.log("in dao " + batterydao.sqlGetAll)
        var con = getConnection(); // we need a different connection for each
        con.connect(function (err) {
            if (err) throw err;
            var data = [];
            con.query(batterydao.sqlGetAll,[] ,function (err, result) {
